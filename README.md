...
- **T2 类别识别**（agent）：识别 pci/net/... 标签（选模板开关；`--category`
  人工覆盖；不可判定回落通用模板+警告）。
- **最小前置依赖**：从 Linux 注册/匹配及调用关系检查目标所需能力，可追到
  输入目录外的提供者。缺失或未确认时生成 `P0/reports/prerequisite_tasks.md`
  及 JSON 任务清单，返回 3 阻塞主驱动；完成前置迁移后重跑 P0，重新核对
  依赖再继续。此步骤不自动迁移依赖，也不改变主驱动的文件范围。
