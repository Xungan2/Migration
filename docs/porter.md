...
prepare 的主 agent 可以做范围判断、接受或拒绝任务结果。任务 agent 根据任务类别完成源码调查、骨架实现或规划工作。知识 agent 持续负责共享知识的一致性，其他 prepare agent 可按需修正文档并说明原因。mono 研究 agent 负责证据和设计，可修正共享材料并说明原因；翻译 agent 完成实现和测试。accept 各 Tier 根据证据准备标准及必要修复，标准和批准范围的修订交人工审批。
...
任务不是固定流水线。主 agent 可以根据证据调整、合并、延期内部目标，或重新派发必要及补充验证。
在报告或任务说明中记录理由、证据、影响和后续条件。目标的 required 值表示当次派发判断，
历史尝试保留；宿主不通过固定重试次数或 retry JSON 结构代替 owner 决策。用户明确要求和阶段验收仍须满足。
...
1. 主 agent 根据证据分别通过骨架和规划验收，并说明历史未完成目标的处理及影响；
2. 知识 agent 已处理当前交接，主 agent 已复核收据；
3. 工作区知识索引、必要主题入口及非空 `migration-plan.md` 存在；
4. 验收输入与证据未失效；
5. `prepare` 成功 handoff 已发布。

历史任务的 blocked 状态保留供追溯，不再独立否决当前有效的 owner 验收。
旧工作区无需迁移；原有 `completed_goals` 验收引用仍可读取和提交。

格式错误、缺少证据或产物、提前 finish 会保存为 `prepare/state.json` 的 `feedback`，包含
原始交付、错误和调用日志路径，再交回 agent。task 和知识 agent 各有一次续接原 session、
仅补交结果的机会；补交仍失败或调用失败时保留 checkpoint，返回 owner 决策。
owner 的可修正错误在剩余预算内反馈给 owner，知识同步失败保持未同步。
传输支持分段文本和唯一 JSON 对象外的说明文字；歧义或损坏输出要求 agent 修正，不猜测成功。

历史交接被改写、删除、移动，或 owner/task 修改共享知识时，宿主生成 `workspace-change`
交接记录（路径、调用日志及可用的前后指纹），交给 owner 复核及知识 agent 同步，不直接退出。
知识维护期间发生的交接变化留到下一批处理；受影响的验收证据仍需复核。变更记录不备份旧文件内容。
预算耗尽、人工中断或无法持久化仍停止执行。
owner 明确提交 blocker 时保存阻塞交接；其他可修正问题不再直接结束整轮 prepare。
...
研究 agent 以证据和设计为主要输出，必要修正须说明理由。它需要阅读模块规格、源代码、目标接口、前序模块结果、契约登记表和泊车记录，形成结构化研究交付物。
...
研究阶段检查模块标识及已提供数据的字段类型，证据路径问题记录为警告。章节组织和裁定用语由 agent 决定，不适用的集合可以省略或为空。交付检查失败在剩余预算内回灌同一 session 修复。
...
核对实际构建登记及必要接线。迁移可复用或减少代码，代码行数不参与验收。范围变化交给 agent 复核用途并更新接线清单；编译、启动及测试仍须真实通过。
...
execute 使用已批准标准执行检查；发现标准错误时可准备候选修订、验证及影响说明，再登记人工审批。它按 §1 到 §7 的顺序执行，第一处失败会进入修复循环或停车。
...
检查 `migrated_functions`、`tests` 和 `untested` 三组数据，确认测试标记真实增加并且豁免有理由。问题在同一 session 内持续反馈，累计使用剩余预算；agent 可以补充测试或说明硬件限制。
...
标准绑定后发生变化时，agent 先准备候选修订和影响说明，完成可行验证，再返回待审批状态（退出码 3）。批准后续接并重新验证，拒绝意见会回到 agent；旧批准不能覆盖修改后的标准。

### 19.11 execute 报范围违规

agent 复核路径及 git diff，说明必要改动用途，准备候选范围和评审材料。涉及已批准范围的扩大先交人工审批，保留现有工作。
...
12. 目标树改动、计划、交接和证据的变化已复核；验收使用当前批准的标准。
...
pre-mono、mono 和 accept 共用可续接执行循环。格式、产物及最终检查失败会反馈给 agent；相似错误只提醒，不按固定次数终止。预算累计计算 agent 调用耗时，重连不重置预算，已有修复宽限只使用一次。外部检查保留独立超时。

普通交接材料的修改、删除或移动会记录路径和指纹差异，由当前 agent 复核。复核期间再次变化需重新确认受影响结论；历史内容不自动恢复。agent 回合与反馈保存在 `.seq.json`，恢复时归档前次日志。真实阻塞、预算耗尽、中断或执行/关键存储不可用会保存可取得的进度并结束；失败检查不会发布成功。
